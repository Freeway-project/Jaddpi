...
import { Mail, Phone, ArrowLeft, Building2, CheckCircle2 } from 'lucide-react';
  ...
  const onSubmit = async (data: BusinessSignupFormData) => {
    ...
    try {
      ...
      if (!storeEmail || !storePhone) {
        toast.error('Both email and phone number are required for business accounts');
      ...
      if (!data.businessName || data.businessName.trim().length < 2) {
        toast.error('Business name is required (minimum 2 characters)');
        setIsSubmitting(false);
        setLoading(false);
        return;
      }
    ...
    <div className="w-full max-w-[400px] mx-auto space-y-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-2">
          <Label htmlFor="businessName" className="text-sm font-medium text-slate-700 ml-1">Business Name</Label>
          ...
          {errors.businessName && (
            <p className="text-sm font-medium text-rose-500 ml-1">{errors.businessName.message}</p>
          )}
        </div>
